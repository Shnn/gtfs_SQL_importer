...
DROP TABLE if exists gtfs_shape_geoms;
...
UPDATE gtfs_stops SET the_geom = SetSRID(MakePoint(stop_lon, stop_lat), 4326);

-- Create spatial index
CREATE INDEX "gtfs_stops_the_geom_gist" ON "gtfs_stops" (the_geom);
...
SELECT shape.shape_id, SetSRID(MakeLine(shape.the_geom), 4326) As new_geom
  FROM (
    SELECT shape_id, MakePoint(shape_pt_lon, shape_pt_lat) AS the_geom
    FROM gtfs_shapes 
...
-- Create spatial index
CREATE INDEX "gtfs_shape_geoms_the_geom_gist" ON "gtfs_shape_geoms" (the_geom);
